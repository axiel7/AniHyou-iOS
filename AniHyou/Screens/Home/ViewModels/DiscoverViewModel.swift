...
class DiscoverViewModel: ObservableObject {
    ...
    func getAiringAnimes(page: Int = 1) {
        ...
        Network.shared.apollo.fetch(query: AiringAnimesQuery(
            ...
            perPage: .some(10),
            sort: .some([.case(.time)]),
            airingAtGreater: .some(todayTimestamp),
            airingAtLesser: .none
        )) { [weak self] result in
            switch result {
    ...
    func getAiringOnMyList() {
        Network.shared.apollo.fetch(query: AiringOnMyListQuery(
            page: .some(1),
            perPage: .some(25)
        )) { [weak self] result in
            switch result {
            case .success(let graphQLResult):
                if let page = graphQLResult.data?.page {
                    if let media = page.media {
                        self?.airingOnMyList = media.filter({ $0?.nextAiringEpisode != nil })
    ...
    func getSeasonAnimes(page: Int = 1) {
        Network.shared.apollo.fetch(query: SeasonalAnimeQuery(
            page: .some(page),
            perPage: .some(10),
            season: .some(.case(nowAnimeSeason.season)),
            seasonYear: .some(nowAnimeSeason.year),
    ...
    func getTrendingAnimes(page: Int = 1) {
        Network.shared.apollo.fetch(query: MediaSortedQuery(
            page: .some(page),
            perPage: .some(10),
            type: .some(.case(.anime)),
            sort: .some([.case(.trendingDesc)])
        )) { [weak self] result in
            switch result {
            case .success(let graphQLResult):
                if let page = graphQLResult.data?.page {
    ...
    func getNextSeasonAnimes(page: Int = 1) {
        Network.shared.apollo.fetch(query: SeasonalAnimeQuery(
            page: .some(page),
            perPage: .some(10),
            season: .some(.case(nextAnimeSeason.season)),
            seasonYear: .some(nextAnimeSeason.year),
            sort: .some([.case(.popularityDesc)])
        )) { [weak self] result in
            switch result {
            case .success(let graphQLResult):
